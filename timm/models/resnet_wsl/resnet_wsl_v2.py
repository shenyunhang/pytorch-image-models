...
from .box_head import DiscriminativeAdaptionNeck
...
default_cfgs = {
    # WSResNet
    'wsl_resnet18': _cfg(url=''),
    'wsl_resnet50': _cfg(url=''),
    'wsl_resnet101': _cfg(url=''),
}
...
class ResNet(Backbone):
    ...
    def __init__(self, stem, stages, num_classes=None, out_features=None, fc_dims=[4096, 4096]):
        """
        Args:
...
def build_wsl_resnet_v2_backbone(depth, res2_out_chans, fc_dims, block, layers, num_classes=1000, in_chans=3,
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        first_stride = 2 if idx == 1 or (stage_idx == 4 and res5_dilation == 1) else 1
        has_pool = True if stage_idx == 3 or stage_idx == 4 else False
        stage_kargs = {
            "num_blocks": num_blocks_per_stage[idx],
            "stride_per_block": [first_stride] + [1] * (num_blocks_per_stage[idx] - 1),
            "has_pool_per_block": [has_pool] + [False] * (num_blocks_per_stage[idx] - 1),
    ...
    return ResNet(stem, stages, num_classes=num_classes, out_features=out_features, fc_dims=fc_dims).freeze(freeze_at)


def _create_resnet(variant, pretrained=False, **kwargs):
    return build_model_with_cfg(
        build_wsl_resnet_v2_backbone, variant, default_cfg=default_cfgs[variant], pretrained=pretrained, **kwargs)


@register_model
def wsl_resnet18(pretrained=False, **kwargs):
    """Constructs a WSResNet-18 model.
    """
    model_args = dict(depth=18, res2_out_chans=64, fc_dims=[4096, 4096], block=BasicBlock, layers=[2, 2, 2, 2], **kwargs)
    return _create_resnet('wsl_resnet18', pretrained, **model_args)
